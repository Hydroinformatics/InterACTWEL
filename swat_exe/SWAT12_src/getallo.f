...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
     ...
     &                      fertdb, subfile, fcstfile, bsnfile, watrgt,
     &                      hruwrf
      character (len=1) ::  a
...
!     septic database file
      ...
      read (23,5000) watrgt !! Added by Sammy
      read (23,5000) hruwrf !! Added by Sammy
...
!! calculate number of records in water rights
      eof = 0
      mwr = 0
      kwr = 0
      open (32,file=watrgt)
...
!! calculate number of records in hru + water rights data
      eof = 0
      mhruwr = 0
      open (33,file=hruwrf)
      do
        itnum = 0
        read (33,6300,iostat=eof) itnum
        if (eof < 0) exit
