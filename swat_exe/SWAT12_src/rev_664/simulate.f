...
!          if (biomix(j) > .001) call tillmix (j,biomix(j))
     ...
     &                     mat_yrs(idplt(j)))
      ...
      !! Write total water rights allocation
      !! do kwr = 1, nhru  
      !! write (9480,5040) kwr,'   ',hruwr(kwr),'   ', hruwra(hruwr(kwr)),
      !! & '   ', iyr
      !! end do

        do ic = 1, kwr
          if (ic <= kwr - 1) then
          write (9480,5040) ic,'   ', hruwra(ic),'   ', iyr
          hruwra(ic) = 0.
          else
          write (9480,5040) 999,'   ', hruwra(999),'   ', iyr
          hruwra(999) = 0.
          end if
        end do
