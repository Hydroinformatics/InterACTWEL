...
!!    cnv2        |none          |conversion factor (m^3 => acres-feet)
...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      integer :: jrch, k, kk, flag, ii
      real :: cnv, vmm, vminmm, vol, wtrin, cnv2
      real :: wrdiff

      integer :: strdate, enddate
     ...
     &            auto_wstr(k)) flag = 2
        !!if (k == 632) then
        !!  WRITE(*,*) k, strsw(k), auto_wstr(k), flag
        !!end if
        end if
        ...
        !!WRITE(*,*) irrsc(k), irrno(k), depthwr(hruwr(k)), hruwra(k)
        if (irrsc(k) == 1 .and. irrno(k) == jrch) then
          ...
          if (flag > 0) then
            ...
            if (rtwtr > flowmin(k) * 86400.) then
              ...
              !! Added by Sammy
              wrdiff = depthwr(hruwr(k))
              if ((vmm * cnv * cnv2) + hruwra(hruwr(k)) > wrdiff) then 
                vmm = depthwr(hruwr(k)) - hruwra(hruwr(k))
                if (vmm < 0.) then vmm = 0.
              end if

              strdate = wrstrdate(hruwr(k))
              enddate = wrendate(hruwr(k))
            ...
            if (vmm > 0.) then
                ...
                !!if (k == 632) then
                !!  WRITE(*,*) "IrrAmt: ", k, irr_mx(k), vmm, vol, flag
                !!end if
       ...
       !!         if (ipot(k) == k) then
                if (pot_fr(k) > 1.e-6) then
                  ...
                  call irrigate(k,vmm)
                  !!if (k == 632) then
                !!WRITE(*,*) k, vol, vmm, (vmm * cnv * cnv2), hru_ha(k)
                  !!end if
...
1000        format (a5,1x,a4,3i6,1x,e10.5,1x,2a15,7f10.2,10x,f10.2,70x,
     *      i10,10x,i10) 
      ...
      !WRITE(*,*) "Line 258"
      !WRITE(*,*) wtrin, rtwtr
      if (abs (wtrin - rtwtr) < 1.e-6 .and. wtrin > 0.01) then
        !WRITE(*,*) "Line 259"
