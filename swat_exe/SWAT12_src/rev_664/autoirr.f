...
!! Added by Sammy
        ...
        wrdiff = depthwr(hruwr(j))
        if ((vmm * cnv * cnv2) + hruwra(hruwr(j)) > wrdiff) then 
          vmm = depthwr(hruwr(j)) - hruwra(hruwr(j))
          if (vmm < 0) then vmm = 0.
        end if
     
        strdate = wrstrdate(hruwr(j))
        enddate = wrendate(hruwr(j))
            ...
            case (3)   !! shallow aquifer source
              do k = 1, nhru
                if (hru_sub(k) == irrno(j)) then
                  ...
                  vmma = vmma / cnv
                  !!WRITE(*,*) "Check point 2", k, irrno(k), vmma, cnv
                  vmma = vmma / irr_eff(j)
