...
class Farmers(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_output('indv_profit', val=np.zeros(self.nactors*self.nyears))
        self.add_output('indv_envir', val=np.zeros(self.nactors*self.nyears))
        self.add_output('const_per', val=np.ones(self.nactors*self.nyears))
            ...
            for ij in self.hrus_areas.keys():
                if wrids+1 == int(ij):
                    ...
                    for yi in range(0,self.nyears):
                        ...
                        outputs['indv_profit'][wrs_yr_ids] = indv_profit    
                        outputs['const_per'][wrs_yr_ids] = total_hru_irr
                        
                        wrs_yr_ids = wrs_yr_ids + 1
                        
                        

                hru_ids = hru_ids + len(self.hrus_areas[int(ij)])*self.nyears
                wrs_yr_ids = wrs_yr_ids + self.nyears
