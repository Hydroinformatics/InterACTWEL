...
class FEWNexus(om.Group):
    ...
    def setup(self):
        ...
        ##des_vars = self.add_subsystem('wr_vols', om.Group(), promotes_inputs=['wr_vols'])
        des_vars.add_output('wr_vols', val=np.ones(self.nactors)*20)
        
        ######## SETUP of FARMERS ###############
        
        #wr_rights = self.add_subsystem('wr_rights', om.IndepVarComp())
        #wr_rights.add_output('wr_vols', val=np.ones(4))
        
        #self.add_subsystem('aggregator', RegionToFarmer())
        #self.connect('wr_rights.wr_vols','aggregator.wr_vols')
        #self.connect('aggregator.crop_wrvols','region.irr_amt')
        ...
        #self.add_output('profit_region',val=0.0)
        ...
        #self.connect('gw_wrvol','region.gw_irr_amt')
        #self.connect('s_wrvol','region.sw_irr_amt')
    ...
    #    output['profit_region'] = sum(inputs['actor_profit'])
...
class Region(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input('wr_vols',val=np.zeros(self.nactors))
        #self.add_input('actor_profit',val=np.zeros(self.nactors))
        self.add_output('profit', val=0.0)
        #self.add_output('enviro_region', val=0.0)
        
    #def setup_partials(self):
        #self.declare_partials('profit_region', 'crops*', method='fd')
        self.declare_partials('profit', 'wr_vols*', method='fd')
    ...
    def compute(self, inputs, outputs):
        ...
        # profit_region = 0
        # for i in range(0,len(inputs['crops'])):
            
        #     irr_amt = inputs['irr_amt'][i]
        #     crop_yield = np.interp(irr_amt, self.w_crops[:,0], self.w_crops[:,i+1]) 
        #     per_yield = 1 + (0.8*-np.exp(self.p_crops_a[i]*irr_amt))
            
        #     profit_region = profit_region + crop_yield*per_yield*self.crops_price[i]*inputs['crops'][i]
        #     #print([i,inputs['irr_amt'][i],inputs['crops'][i],profit_region])
            
        # outputs['profit_region'] = profit_region
...
class FarmerOpt(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        print('subopt ' + str(self.farmer_id) +' solve')
        p.run_driver()
...
class Farmer(om.ExplicitComponent):
    ...
    def initialize(self):
        ...
        # self.options.declare('p_crops_a', default=[-.3,-.1,-.05])
        self.p_crops_b = [4000,5000,3000]
        
        # self.options.declare('p_crops_b', default=[4000,5000,3000])
        
        self.f_cost = [500,400,200]
        # elf.options.declare('f_cost', default=[500,400,200])

        # self.options.declare('crops_price', default=[200,500,100])
        self.cost_fert = self.calculate_cost_fert()
        # cost_fert = self.calculate_cost_fert()
        # self.options.declare('cost_fert', default=cost_fert)        
    ...
    def setup(self):
        ...
        #self.add_output('invd_envr', val=0.0)
        self.add_output('indv_profit', val=0.0)
        self.add_output('total_irr', val=0.0)
    ...
    def compute(self, inputs, outputs, discrete_inputs, discrete_outputs):
        ...
        if abs((sum(inputs['hru_irr']) - 100.0)) > 0.0:
            ...
            for i in range(0,len(self.hrus_areas)):
                ...
                profit = crop_yield*per_yield*self.crops_price[self.hrus_crops[i]-1]*self.hrus_areas[i] - cost_f # profit function        
