...
!!      do ihru = 1, nhru
        do ihruk = 1, nhru !! Change by Sammy to consider priority of WR
          ihru = priorwr(ihruk) !! Change by Sammy to consider priority of WR                                     
...
!          if (biomix(j) > .001) call tillmix (j,biomix(j))
     ...
     &                     mat_yrs(idplt(j)))
        ...
        do ic = 1, kwr
          if (ic <= kwr - 1) then
          write (9480,5040) ic,'   ', hruwra(ic,curyr),'   ', iyr
          hruwra(ic,curyr) = 0.
          else
          write (9480,5040) 9999,'   ', hruwra(9999,curyr),'   ', iyr
          hruwra(9999,curyr) = 0.
          end if
        end do
        
        do ic = 1, uhruwr
          if (hruwr_dict(ic,2) /= 9999) then
          write (9481,5041) hruwr_dict(ic,1),'   ', hruwr_dict(ic,2),
     &     '   ', hruwr_dict(ic,3),'   ', hruwr_dict(ic,4),'   ', 
     &     hruwruse_dict(ic),'   ', iyr 
 ...
 5040 format (i4,a,f16.10,a,i4) !! Added by Sammy
 5041 format (i6,a,i6,a,i4,a,i4,a,f16.10,a,i4) !! Added by Sammy
      end