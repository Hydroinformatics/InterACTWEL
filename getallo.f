...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
     ...
     &                      hruwrf, strwrf
      ...
      integer :: icd, inm1, inm2, inm3, iht, eof, numhru, ic, iniwr
...
!! calculate number of records in water rights !!Added by Sammy  
      eof = 0
      mwr = 0
      kwr = 0
      wryr = 0
      ...
      do
        itnum = 0
        iniwr = 0
        read (32, 6500, iostat=eof) itnum, iniwr
        if (eof < 0) exit
        if (itnum == 1 .and. iniwr > 0) then
          kwr = kwr + 1
          !!WRITE(*,*) 'Allocation vars', kwr, iniwr
          mwr = Max(mwr,iniwr) 
        endif
        if (itnum > 0) then
          wryr = Max(wryr,itnum)
      ...
      !!WRITE(*,*) 'Allocation vars', mwr, wryr
      
      close (32)

!! calculate number of records in hru + water rights data !!Added by Sammy 
      eof = 0
      mhruwr = 0
      uhruwr = 0
      open (33,file=hruwrf)
