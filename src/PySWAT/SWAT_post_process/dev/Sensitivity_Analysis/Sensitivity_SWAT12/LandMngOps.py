...
class LndMngOps(object):
    ...
    def FindString(self, startstr, endstr, line):
        ...
        with open(self.mngops_parmt_file,'rb') as search:
            for line in search:
                if '#' in line:
                    ...
                    param_name = self.FindString('#',':',linesplit)
                    param_values = self.FindString('[',']',linesplit)
                ...
                if operations_linebool == 1:
                    for crop_id in hru_crops[hruid]:
                        ...
                        for schd_line in self.MngSched[schd_oper].keys():
                            ...
                            if len(opkey) > 0:
                                temp_line = self.AddParamsToLine(temp_line, self.MngParams[opkey[0]], sub_basin)
                                wrt.write(temp_line + '\n')
                            else:
                                wrt.write(temp_line + '\n')
                    operations_linebool = 0
...
#%%    
    def ChangeHRU_Mgt(self, hruids, hru_files, hru_crops):
        ...
        for hru in hruids:
            ...
            self.Write_MgtSchd(file_path, hru, hru_crops)
