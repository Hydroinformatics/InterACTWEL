...
#%% Get file names of HRUs and respective (SUB,HRU) ids  & HRU to HRU_IDS in SWAT
...
#%% Get baseline data and file of HRUs and map to their Subbasin
def Get_HRUs_data(tfile,var):
    output_array = dict()
    templulc = []
    temphru = [] 
    tempsub = []
...
#%% Get array of time steps used to write output SWAT files
...
#%% Get output data from Main Channel Output file (.rch)
...
#%% Get output data from HRU IMPOUNDMENT Output file (.wtr)
...
#%% Get output data from HRU Output file (.hru)
...
#%% Get baseline output data based on opt. objectvies
...
#%% Get baseline values of decision variables
...
class SWATmodel():
    def __init__(self,path,Problem):
        ...
        self.HRUs = Get_HRUs_data(self.path['SWAT'] + 'TxtInOut/output.hru','HRU')
...
#    path = 'Z:/Projects/INFEWS/Modeling/TestProblem/Model/Scenarios/Default.zip'
    pathuzip = '/export/Gateway_SWAT'
    UnzipModel(args.path[1],pathuzip)

##%% Setup problem and prase SWAT baseline data
    path = dict()
    path['SWAT'] = '/export/Gateway_SWAT/Default/'
    path['PROB'] = args.path[0]
    path['ZIP'] = args.path[1]
    
    temp_folder = '/export/Gateway_SWAT/GA'
    if os.path.isdir(temp_folder):
        shutil.rmtree(temp_folder)
    os.makedirs(temp_folder)
    path['UNZIP'] = temp_folder
    # Read optimization formulation file and create object for Opt. Formulation
    OptProblem = Formulation.OptFormulation(path)
    # Read and create SWAT object
    SWATmodel = SWATmodel(path,OptProblem)
#    print SWATmodel.hrus_file_id
...
#prob.driver.opt_settings['PopSize'] = 5
