...
def Fitness_Function(unzip, pzip, inputs):
    """
    Define the function f(xI, xC).

    When Branin is used in a mixed integer problem, x0 is integer and x1 is continuous.
    """
#    _SWATmodel = copy.deepcopy(SWATmodel)
#    _optproblem = copy.deepcopy(optproblem)
...
#    SWATmodel = self.metadata['SWAT']
    ...
    if current.name is 'MainProcess':
        iter_count = len(os.walk(unzip).next()[1])
    else:
        iter_count = current._identity[0]
    
    temp_path = UnzipModel(pzip,unzip,iter_count)
...
#    #os.chdir(temp_path +'/TxtInOut/')
    ...
    try:
        ...
        tempcwd = os.getcwd()
        os.chdir(temp_path +'/TxtInOut/')
        #yexitflag = subprocess.check_call(['swatmodel_64rel.exe'])
...
#                            outputs = Get_fitness_region(obje,bvar,optproblem, SWATmodel, output_data)
#                            
...
##                                     outputs[output_name] = Get_fitness_hru(obje,bvar,optproblem, SWATmodel, output_data)
        ...
        outputs = inputs[0]*np.random.rand()     
        print outputs                 
    except:
