...
from Objective_Functions_par import Fitness_Function

#%%
def instance_method_alias(obj, arg):
    """
    ...
    #arg = list(arg)
    print arg
    fit_val = obj.objective_callback(arg)
...
class DeapGADriver(Driver):
    ...
    def __init__(self):
        ...
        self.options.declare('Problem',default = None)
        self.options.declare('SWAT',default = None)
...
#            raise RuntimeError(msg)
        ...
        if self.options['run_parallel']:
            ...
            self.comm = self.options['run_parallel']
        
        #bound_instance_method_alias = functools.partial(instance_method_alias, self)
        #self._ga = NSGAAlgorithm(bound_instance_method_alias, comm=comm)
    ...
    def run(self):
        ...
        desvar_new, obj, nfit, hof, log, pop = self.execute_ga(lower_bound, upper_bound, bits, pop_size, max_gen, wghts, optprint)
...
#        self.iter = 0
    ...
    def execute_ga(self, vlb, vub, bits, pop_size, max_gen, wghts, optprint):
        ...
        optproblem = self.options['Problem']
        SWATmodel = self.options['SWAT']
        ...
        if self.comm:
            ...
            toolbox.register("evaluate", Fitness_Function, SWATmodel.path['UNZIP'], SWATmodel.path['ZIP'])
            #toolbox.register("evaluate", self.objfun(None))
            
        else:
            #toolbox.register("evaluate", objective_callback, self)
            print SWATmodel.path['UNZIP']
            toolbox.register("evaluate", Fitness_Function, SWATmodel.path['UNZIP'], SWATmodel.path['ZIP'])
...
def objective_callback(obje, x):
    ...
    int
        Case number, used for identification when run in parallel.
    """
    objec = copy.deepcopy(obje)
    model = objec._problem.model
    success = 1
    
    for name in objec._designvars:
        i, j = objec._desvar_idx[name]
        objec.set_design_var(name, x[i:j])

    # Execute the model
    with Recording('DeapGA', objec.iter_count, obje) as rec:
        objec.iter_count += 1
        try:
            model._solve_nonlinear()
            model._outputs
...
#                    print(val)
        ...
        obj = []
        for name, val in iteritems(objec.get_objective_values()):
            obj.append(val[0])
            #break
