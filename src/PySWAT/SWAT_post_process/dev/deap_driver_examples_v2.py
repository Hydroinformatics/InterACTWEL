...
#        self.iter = 0
    ...
    def execute_ga(self, vlb, vub, bits, pop_size, max_gen, wghts, optprint):
        ...
        #bound_instance_method_alias = functools.partial(instance_method_alias, objective_callback, self)
...
def objective_callback(obje, x):
    ...
    #obje = copy.deepcopy(objec)
    model = obje._problem.model
    success = 1
    popid = obje.iter_count

    penalty = []
    for name in obje._designvars:
        i, j = obje._desvar_idx[name]
        obje.set_design_var(name, x[i:j])
        penalty.append(FeasiblePt(obje,name, x[i:j][0]))
            
    # Execute the model
    #with Recording('DeapGA', obje.iter_count, obje) as rec:
    obje.iter_count += 1
    try:
        model._solve_nonlinear()
        model._outputs
...
#                    print(val)
    ...
    obj = []
    for name, val in iteritems(obje.get_objective_values()):
        obj.append(val[0])
        #break
...
def FeasiblePt(obje, name, xind):
    for tconst in obje._cons:
        if tconst == name:
            if xind >= obje._cons[tconst]['lower'] and xind <= obje._cons[tconst]['upper']:
                return 1000
    return 0