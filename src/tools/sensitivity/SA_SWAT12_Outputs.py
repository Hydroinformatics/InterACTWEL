...
#%%    
#    def WriteOutputCSV(self, outputcsv_writer, num_sim):
#        for var in self.output_data.keys():
#            unit_type = self.output_data[var]['Type']
#            for unit_ids in self.output_data[var].keys():
#                #for val in range(0,len(self.output_data[var][unit_ids])):
#
#                outputcsv_writer.writerow([num_sim,var,unit_type,unit_ids,self.output_data[var][unit_ids]])
...
#  
    ...
    def Get_output_std(self, tfile, table, var, varcol):
        ...
        return data_array
#        
#    def Get_Rout_Ele_table(self,db_path):
