...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      integer :: jrch, k, flag, ii, kk, wrid
      real*8 :: cnv, vmm, vminmm, vol, wtrin
      !real*8 :: wrdiff, cnv2, vmm2 !! Added by Sammy
      integer :: strdate, enddate !! Added by Sammy
     ...
     &             auto_wstr(k)) flag = 2
      ...
      !!endif
        ...
        if (irrsc(k) == 1 .and. irrno(k) == jrch) then
          ...
          if (flowmin_wr(jrch, wrdate) > 0) then
            flowmin(k) = flowmin_wr(jrch, wrdate)
          endif
          ...
          if (flag > 0) then
            ...
            if (rtwtr > flowmin(k) * 86400.) then
              cnv = 0.
              cnv = hru_ha(k) * 10.
              !cnv2 = 35.3147 * (1. / 43560.) !! Added by Sammy

              vmm = 0.
              vminmm = 0.
              !vmm2 = 0. !! Added by Sammy
    ...
    !          vmm = Min(vmm2, vmm)
              ...
              wrid = hruwr(k)
              strdate = wrstdate(wrid,curyr)
              enddate = wrendate(wrid,curyr)
              if (wrdate < strdate .or. wrdate > enddate) then
                vmm = 0.
              end if
  ...
  !! Added by Sammy
       ...
       !!         if (ipot(k) == k) then
                ...
                !end if
                  ...
                  call allocatewr(k,vmm) !! Added by Sammy
...
1000        format (a5,1x,a4,3i6,1x,e10.5,1x,2a15,7f10.2,10x,f10.2,70x,
     *      i10,10x,i10) 
      ...
      !! Added by Sammy
      !!WRITE(*,*) "Starting irr_rch", k, bool_priorwr
      if (bool_priorwr == 1) then 
